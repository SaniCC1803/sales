  ...
  return (
    ...
    >
      ...
      <div className={`p-4 ${isUser(item) ? 'py-6' : ''} flex flex-col flex-1`}>
        ...
        <CardDescription className="pb-2">{getDisplayDescription()}</CardDescription>
        {/* Subcategories */}
        {isCategory(item) && item.subcategories?.length ? (
          <CardDescription>
            <span className="font-bold">Subcategories:</span>{' '}
            {item.subcategories
              .map((sub) => sub.translations?.find((t) => t.language === language)?.name || '')
              .join(', ')}
          </CardDescription>
        ) : null}
        ...
        {/* Buttons */}
        <div
          className={`${isUser(item) ? 'mt-6' : 'mt-4'} flex gap-2 items-center justify-between`}
        >
          {isAdmin ? (
